...
pizzaJson.map((item, index)=> {
  ...
  pizzaItem.querySelector('.pizza-item--price').innerHTML = `$ ${item.prices[priceIndex].toFixed(2)} ${item.sizes[priceIndex]}`;
  ...
  pizzaItem.querySelector('.pizza-item a').addEventListener('click', (e)=>{
    ...
    s('.pizzaInfo--actualPrice').innerHTML = `$ ${pizzaJson[key].prices[priceIndex].toFixed(2)}`;
...
s('.cart--finalizar').addEventListener('click', ()=>{
  ...
  setTimeout(()=>{
    s('.forecast').innerHTML = 'Tiempo de entrega: 30 minutos';
...
function updateCart() {
  ...
  s('.discount span:last-child').innerHTML = `$ ${discount.toFixed(2)}`;
  s('.total span:last-child').innerHTML = `$ ${total.toFixed(2)}`;
  } else {
...
function updatePrice() {
  s('.pizzaInfo--actualPrice').innerHTML = `$ ${(pizzaJson[modalKey].prices[priceIndex]*modalQt).toFixed(2)}`;
} 