...
def index():
    ...
    id = session["user_id"]
    database = db.execute("SELECT * FROM portfolio WHERE id = ? GROUP BY symbol", id)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if (price * share) > user_cash:
            ...
            if check:
                old_share = db.execute("SELECT shares FROM portfolio WHERE id = ?", session["user_id"])
                new_share = old_share + share
                ...
                db.execute("INSERT INTO portfolio (id, symbol, stock_name, shares, price, total_price) VALUES (?, ?, ?, ?, ?, ?)", session["user_id"], dic["symbol"], stock_name, share, price, total_price)

            db.execute("INSERT INTO history (id, symbol, shares, price, total_price, operation, transacted) VALUES (?, ?, ?, ?, ?, ?, ?)", session["user_id"], dic["symbol"], share, price, total_price, "BUY", dt_string)
...
def history():
    ...
    id = session["user_id"]
    database = db.execute("SELECT * FROM history WHERE id = ?", id)
...
def sell():
    ...
    sell_symbol = db.execute("SELECT symbol FROM portfolio WHERE id = ? GROUP BY symbol", session["user_id"])
    ...
    if request.method == "POST":
        ...
        shares_owned = db.execute("SELECT shares FROM portfolio WHERE symbol = ? AND id = ? GROUP BY symbol", symbol, user_id)[0]["shares"]
        ...
        if shares_owned - shares == 0:
            db.execute("DELETE FROM portfolio WHERE symbol = ? AND shares = ? AND id = ?", symbol, shares_owned, user_id)
        else:
            new_shares = shares_owned - shares
            db.execute("UPDATE portfolio SET shares = ?, total_price = ? WHERE id = ?", new_shares, new_shares * item_price ,user_id)

        now = datetime.now()
        dt_string = now.strftime("%d-%m-%Y %H:%M:%S")

        db.execute("INSERT INTO history (id, symbol, shares, price, total_price, operation, transacted) VALUES (?, ?, ?, ?, ?, ?, ?)", user_id, symbol, -shares, item_price, total_price, "SELL", dt_string)
